...
function postchild(message: WanixTermProbeMsg) {
...
function probecall<T>(method: string, args: unknown[] = []): Promise<T> {
  ...
  return new Promise<T>((resolve, reject) => {
    ...
    postchild({ type: 'zss-wanix-term-probe-rpc', id, method, args })
  })
}
...
async function runsmoke() {
  ...
  window.addEventListener('message', (event) => {
    ...
    if (!iswanixtermprobemsg(data)) {
      return
    }
    if (data.type === 'zss-wanix-term-probe-rpc-res') {
  ...
  log('child ready — waiting for login prompt…')
