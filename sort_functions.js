...
function extractFunctions(content) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    inString = lineInString ? lineStringChar : (inString && !lineInString ? null : inString);
    inComment = lineInComment || (inComment && !trimmed.includes('*/'));
    
    // Check for function start
    if (!inString && !inComment && trimmed.match(/^export\s+(?:async\s+)?function\s+\w+/)) {
      // Save previous function if exists
      if (currentFunction) {
        functions.push(currentFunction);
      }
      currentFunction = {
        name: trimmed.match(/^export\s+(?:async\s+)?function\s+(\w+)/)?.[1] || '',
        lines: [line],
        startIndex: i
      };
      braceDepth = 0;
