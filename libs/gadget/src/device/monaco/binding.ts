            ...
            if (
              ...
              if (
                ...
                if (anchorAbs.index < headAbs.index) {
                  ...
                  afterContentClassName = 'y-remote-selection-head'
                  beforeContentClassName = null
                } else {
                  start = monacoModel.getPositionAt(headAbs.index)
                  end = monacoModel.getPositionAt(anchorAbs.index)
                  afterContentClassName = null
                  beforeContentClassName = 'y-remote-selection-head'
                }
                newDecorations.push({
                  range: new monaco.Range(
                    start.lineNumber,
                  ...
                  options: {
                    className: 'y-remote-selection',
                    afterContentClassName,
                    beforeContentClassName,
                    hoverMessage: { value: state.name ?? 'Anonymous' },
                    minimap: {
                      color: 'orange',
...
export function monacoBinding(
  ytext: Y.Text,
  editor: monaco.editor.IStandaloneCodeEditor,
) {
  // if (ytext.doc?.meta?.net) {
  const model = editor.getModel()
  // console.info('new MonacoBinding', {
  //   ytext,
  //   editor,
  //   model,
  //   doc: ytext.doc,
