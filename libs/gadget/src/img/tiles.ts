...
import { getThreeColor } from './colors'
...
export const flatMaterial = new THREE.ShaderMaterial({
  // settings
  transparent: true,
  uniforms: {
    time,
  ...
  for (let y = 0; y < height; y += 1) {
    for (let x = 0; x < width; x += 1) {
      ...
      if (empty === true) {
        ...
        const bottom = imageHeight - ty * TILE_IMAGE_STEP - TILE_PADDING
        const top = bottom - TILE_IMAGE_SIZE
...
export function makeTiles(
  ...
  let index = 0
  let offset = 0
  const ox = 0 // width * TILE_SIZE * -0.5 tiles have 0, 0 at top
  const oy = 0 // height * TILE_SIZE * 0.5
  const cols = Math.floor(imageWidth / TILE_IMAGE_STEP)
  ...
  for (let y = 0; y < height; y += 1) {
    ...
    for (let x = 0; x < width; x += 1) {
      ...
      positions.push(px, py + TILE_SIZE, 0)
      positions.push(px + TILE_SIZE, py + TILE_SIZE, 0)
      positions.push(px + TILE_SIZE, py, 0)
