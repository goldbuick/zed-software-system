---
title: "Multiplayer, workers, PeerJS, and WASM runtime"
---
...
Each tab has **three hubs** ([`hub.ts`](../../zss/hub.ts) per realm). WASM is **not** a hub device — planned [`wasm-bridge`](./wasm-sim-port.mdx) loads **`zss_runtime.wasm`**.
