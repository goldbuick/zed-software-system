...
describe('memoryreadelement', () => {
  ...
  it('finds object by exact id', () => {
    ...
    expect(memoryreadelement(board, OBJECT_ID)).toBe(object)
    expect(memoryreadelement(board, OBJECT_ID, { layer: 'object' })).toBe(object)
  })

  it('resolves terrain by numeric board index', () => {
    const board = makeboard()
    expect(memoryreadelement(board, '1')).toBe(terraincell)
    expect(memoryreadelement(board, String(1))).toBe(terraincell)
    // index 0 empty -> undefined terrain slot
    expect(memoryreadelement(board, '0')).toBeUndefined()
