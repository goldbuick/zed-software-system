...
describe('star even-tick dest script', () => {
  it('captures walk seek dest and sends shot, not flow shot', () => {
    ...
    expect(STAR_CODE).toMatch(/#walk seek/)
    expect(STAR_CODE).toMatch(/#set p5 thisx \+ stepx/)
    expect(STAR_CODE).toMatch(/#set p6 thisy \+ stepy/)
    expect(STAR_CODE).toMatch(/#send at p5 p6 shot/)
