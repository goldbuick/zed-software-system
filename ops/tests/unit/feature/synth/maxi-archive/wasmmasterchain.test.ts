...
describe('wasmmasterplaycode', () => {
  it('includes duck, compressor, razzle, and masterout', () => {
    ...
    expect(WASM_SYNTH_VOICE_PLAY_CODE).not.toContain(
      'WASM_PERF_MODE && cnt > 2',
    )
    expect(WASM_SYNTH_VOICE_PLAY_CODE).toContain('var activevoicemask = 0')
    expect(WASM_SYNTH_VOICE_PLAY_CODE).toContain('function readvibratosab')
    expect(WASM_SYNTH_VOICE_PLAY_CODE).toContain('function vibratodepthat')
