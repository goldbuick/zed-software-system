...
describe('zzt import remaps + kind headers', () => {
  ...
  it('star uses #walk seek capture and ?by p3 p4', () => {
    ...
    expect(star).toMatch(/#send by p3 p4 shot/)
    expect(star).toMatch(/any by p3 p4 player/)
  ...
  it('lion/tiger/shark capture walk dest then ?by', () => {
    const lion = readkindzss('lion')
    expect(lion).toMatch(/#walk rnd p2 p3/)
    expect(lion).toMatch(/#walk seek p2 p3/)
    expect(lion).toMatch(/#send by p2 p3 shot/)
    expect(lion).toMatch(/\?by p2 p3/)
    expect(lion).not.toMatch(/\?seek/)
    expect(lion).not.toMatch(/\?rnd/)
    const tiger = readkindzss('tiger')
    expect(tiger).toMatch(/#walk rnd p7 p8/)
    expect(tiger).toMatch(/#walk seek p7 p8/)
    expect(tiger).toMatch(/#send by p7 p8 shot/)
    expect(tiger).toMatch(/\?by p7 p8/)
    expect(tiger).not.toMatch(/\?seek/)
    const shark = readkindzss('shark')
    expect(shark).toMatch(/#walk rnd p2 p3/)
  ...
  it('ruffian dest-checks player before walk step', () => {
    const ruffian = readkindzss('ruffian')
    expect(ruffian).toMatch(/#walk seek p3 p4/)
    expect(ruffian).toMatch(/#walk rnd p3 p4/)
    expect(ruffian).toMatch(/#send by p3 p4 shot/)
    expect(ruffian).toMatch(/#send by stepx stepy shot/)
    expect(ruffian).toMatch(/:thud\n#if any at senderx sendery player do/)
    expect(ruffian).toMatch(/:touch\n#send at senderx sendery shot\n#die/)
