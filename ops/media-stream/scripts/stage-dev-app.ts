 ...
 * show "Zed Cafe Media Stream" instead of generic Electron.
 *
...
const root = MS_ROOT
const productname = 'Zed Cafe Media Stream'
const appname = `${productname}.app`
const electrondir = path.join(root, 'node_modules', 'electron')
const distdir = path.join(electrondir, 'dist')
const pathfile = path.join(electrondir, 'path.txt')
const stampfile = path.join(electrondir, '.ms-dev-brand')
...
if (!existsSync(stockapp) && !existsSync(brandedapp)) {
  console.error(`missing ${stockapp} -- run yarn install in ops/media-stream`)
  process.exit(1)
}
