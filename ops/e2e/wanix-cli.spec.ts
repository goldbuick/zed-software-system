...
async function bootwanixe2e(page: import('@playwright/test').Page) {
  await page.goto('/?ZSS_E2E=1')
  await expect(page.locator('#frame')).toBeVisible()

  await page.waitForFunction(
    () =>
      typeof (window as WindowWithE2e).__zss_e2e?.runwanixsmoke === 'function',
    undefined,
    { timeout: 120_000 },
  )
...
test.describe('wanix CLI integration', () => {
  ...
  test('drop echo_stdin.wasm echoes stdin line', async ({ page }) => {
    await bootwanixe2e(page)

    const report = await page.evaluate(async () => {
      return (window as WindowWithE2e).__zss_e2e!.runwanixstdinsmoke(90_000)
    })

    // eslint-disable-next-line no-console
    console.log(
      '\nwanix stdin smoke report:\n',
      JSON.stringify(report, null, 2),
    )
    ...
    expect(report.sawsandbox, report.logs.join('\n')).toBe(true)
    expect(report.sawstdin, report.logs.join('\n')).toBe(true)
    expect(report.logs.join('\n')).toContain('echo: hello')
    expect(report.sawexit, report.logs.join('\n')).toBe(true)
    expect(report.sawerror, report.logs.join('\n')).toBe(false)
  })

  test('drop hello-repl.wasm accepts two stdin lines', async ({ page }) => {
    await bootwanixe2e(page)

    const report = await page.evaluate(async () => {
      return (window as WindowWithE2e).__zss_e2e!.runwanixreplsmoke(90_000)
    })

    // eslint-disable-next-line no-console
    console.log('\nwanix repl smoke report:\n', JSON.stringify(report, null, 2))
    ...
    expect(report.sawsandbox, report.logs.join('\n')).toBe(true)
    expect(report.sawstdin, report.logs.join('\n')).toBe(true)
    const joined = report.logs.join('\n')
    expect(joined).not.toMatch(/Hello,\s*!/)
    expect(joined).toContain('Hello, alice')
    expect(joined).toContain('You said: world')
    expect(report.sawexit, joined).toBe(true)
    expect(report.sawerror, joined).toBe(false)
  })
})
