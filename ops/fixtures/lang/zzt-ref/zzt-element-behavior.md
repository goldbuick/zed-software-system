...
- **Cafe now:** `#walk rnd p2 p3` / `#walk seek p2 p3` capture, then `#if any by p2 p3 player` `#send by p2 p3 shot` `#die` else `?by p2 p3`. `:touch` send-shot then `#die`. `:shot` score 1 `#die`. Dest is player only (not kind `breakable`).
- **Status:** ok. Dest pre-check avoids shoving a pushable player past `BoardAttack`.
...
- **Cafe now:** fire block unchanged (`p3` bullet/star), then lion-style `#walk rnd|seek p7 p8` dest player `#send by p7 p8 shot` `#die` else `?by p7 p8`. `:touch` send-shot `#die`. `:shot` score 2 `#die`.
- **Status:** ok. Splitting firing-type into `p3` is a flagged deviation (ZZT overloads the P2 high bit).
...
- **Cafe now:** RoZZT shape in one `:think` -- `p2`/`p3` as clamped deltaX/deltaY, then Movement: `#if any by p2 p3 player` / `breakable` (kind `breakable` when that terrain page is loaded, not `@isbreakable`) then `#send by p2 p3 shot` + `#die` else `?by p2 p3`. `:touch` `#send at senderx sendery shot` then `#die`. No `@light`. `:shot` score 1 `#die`.
...
- **Cafe now:** rest/rush with `#walk seek|rnd p3 p4` capture, dest player `#send by` `#die` else `#walk by p3 p4`; while rushing dest-check `by stepx stepy` before auto-step. `:thud` / `:touch` send-shot `#die`. `:shot` score 2 `#die`.
- **Status:** ok. Dest pre-check plus thud/touch covers BoardAttack and player-walks-in.
...
- **Cafe now:** `@isswimming`, `@notbreakable`, `#walk rnd|seek p2 p3` dest player `#send by p2 p3 shot` `#die` else `?by p2 p3`. `:touch` send-shot `#die`. No `:shot`.
...
- **Cafe now:** head-driven same-think drag (no TickProc, no `:preparefollow` / `:dofollow` cascade). All scratch state lives in element stats `p1`-`p10` (never player flags -- those are shared across objects and corrupt multi-centipede boards). Slot map: `p1` intel, `p2` deviance, `p3`=follower id, `p4`=leader id, `p5`/`p6`=step delta then drag scratch, `p7`/`p8`=vacated cell, `p9`/`p10`=cursor cell. **ZZT import** maps `Follower`/`Leader` onto `p3`/`p4` (and `Leader < -1` onto `p5=17` so the 16-think grace fires on the next segment tick). Head `:think` picks an explicit `p5`/`p6` delta (intel/deviance roll -> random cardinal, else seek one axis), one random retry when `blocked by p5 p6`, adopts a trailing segment at `this - delta`, attacks when the dest cell holds the player, else `#pset idle x|y` (moves without yielding, unlike `#go` -- no walk command, so `everytick` never auto-steps the head), then `#repeat 32` that `#pset`s each follower into the cell the node ahead vacated. Segments are fully passive: probe `"$p4"` into `p6` (guarded by `#if p4`), reset `p5` while live, else `#give p5` and `#morph head` after `p5 above 16` (~32 engine ticks at `@cycle 2`), clearing `p4`/`p5` first. Melee via `:thud`/`:touch` `#send … shot`. Cafe `#morph` is in-place kind change (same id/stats, swap code).
- **Status:** ok (parity-noted). Deliberate simplifications vs `ELEMENTS.PAS`: no alignment-only chase (manual seek approximates it), no cw/ccw/opp turn cascade, and **no full-chain reverse when boxed in** -- a stuck head re-rolls a random delta instead of flipping the chain, so only a fully enclosed 1-tile pocket holds it. Add the reverse back if stuck centipedes show up in play. After editing kind codepages, `#restart` (chips do not hot-reload). A/B: θ drags ○ same tick; deviance rooms differ; mid-body shot waits ~16 segment thinks before the orphan promotes.
