...
- **Cafe now:** script-only chain via element stats `p3`..`p9` (head keeps ZZT `p1` intel / `p2` deviance). Mapping: `p3`=follower id, `p4`=leader id, `p5`=linkgrace, `p6`/`p7`=follow dest xy, `p8`/`p9`=head prev xy. **ZZT import** maps status `Follower`/`Leader` indices onto `p3`/`p4` (and `Leader < -1` onto `p5=1`); export writes them back. Script layout: message labels sit **above** `:think`, with boot `#think`/`#end`. Head claims adjacent orphans (`:trylink`), uses `#pset "$p3" p6/p7` after a successful step then `:dofollow` cascade for drag. Stuck: full chain `:fullreverse` (swap `p3`/`p4` along the follower walk; old tail `#become head`). Segments are **passive** on `:think` (only `p5` grace → `#become head`); `:trylink` / `:preparefollow` / `:dofollow` / `:fullreverse` are message handlers only. Weave-like `#pget`/`#pset` (dir or id) support remote param writes from the head. Melee via `:thud`/`:touch` `#send … shot`.
- **Status:** ok (parity-noted). Intentional deviations: stuck promotes immediate follower only (not full chain reverse); rely on `:leaderdied` rather than polling a dead leader id.
