...
- **Cafe now:** script-only chain via element stats `p3`..`p9` (head keeps ZZT `p1` intel / `p2` deviance). Mapping: `p3`=follower id, `p4`=leader id, `p5`=linkgrace, `p6`/`p7`=follow dest xy, `p8`/`p9`=head prev xy. **ZZT import** maps status `Follower`/`Leader` indices onto `p3`/`p4` (and `Leader < -1` onto `p5=1`); export writes them back. Head (and linked segments with empty `p3`) also walk N/S/E/W and `:trylink` unclaimed segments at runtime. Before move, `:preparefollow` records dest cells; after a successful step, `:dofollow` walks `#go toward p6 p7`. Stuck: try `?rnd`, then promote **immediate** follower (`:becomehead`) and `#become segment`. Death sends `:leaderdied` so the next segment promotes. Orphan segments use a **two-think `p5` grace** before `#become head`. Melee via `:thud`/`:touch` `#send … shot`.
