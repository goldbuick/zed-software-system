...
- **Cafe now:** script-only chain via custom `leader`/`follower` **on each object element** (non-player `#set`/`#get` must not use the player flag bag). Head (and linked segments with empty `follower`) walk N/S/E/W and `:trylink` unclaimed segments; `:trylink` recursively extends so a full adjacent chain binds in one send cascade (`:acceptlink` on head and segment). Before move, `:preparefollow` records dest cells down the chain; after a successful step, `:dofollow` walks `#go toward destx desty` and cascades. Stuck: try `?rnd`, then promote **immediate** follower (`:becomehead`) and `#become segment`. Death sends `:leaderdied` so the next segment promotes. Orphan segments use a **two-think `linkgrace`** (mirror ZZT `Leader` -1 then -2) before `#become head`, so heads can `:trylink` first. Melee via `:thud`/`:touch` `#send … shot`.
- **Status:** ok (parity-noted). Intentional deviations: adjacency link at runtime (not ZZT load-time indices); stuck promotes immediate follower only (not full chain reverse); rely on `:leaderdied` rather than polling a dead leader id.
