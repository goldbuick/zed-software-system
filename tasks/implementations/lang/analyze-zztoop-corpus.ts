/**
 * Corpus analyzer for the vanilla-ZZT `zss/feature/zztoop` parser. Forked from
 * `analyze-zzt-corpus.ts`; it differs only in the `compileparse` import and the
 * report/fixture output paths. Lets us track zztoop parse ok-rate against the
 * Museum corpus independently of the ZSS lang parser.
 */
import {
  existsSync,
  mkdirSync,
...
async function main() {
  const code = await analyzezztoopcorpus(process.argv.slice(2))
  process.exit(code)
}
