...
1. `yarn lang:build-train-corpus` — writes `ops/fixtures/generated/training/{train,eval}.jsonl`
...
```bash
pip install -r tasks/implementations/lang/finetune/requirements.txt
...
1. Upload `ops/fixtures/generated/training/train.jsonl` as a dataset or clone repo in job.
2. Use `tasks/implementations/lang/finetune/train.py` as job command with GPU hardware.
