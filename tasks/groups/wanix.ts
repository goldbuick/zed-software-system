...
const WANIX_VM_VALIDATE_TIMEOUTS = {
...
function runwithscripttimeout(ms, label, fn) {
  let timer
...
async function focuscanvas(page, x = 640, y = 400) {
  await page.mouse.click(x, y)
  await page.waitForTimeout(400)
}

async function waitforvmshell(page, log, ms = WANIX_VM_VALIDATE_TIMEOUTS.VM_SHELL_MS) {
  return waitfor(
    page,
    'vm shell',
...
async function waitforzedcafeexportapilog(
  page,
  log,
  key = '__wanixVmApilog',
...
async function assertguestzedcafe(page, log, opts = {}) {
  const {
...
async function readapilog(page, key = '__wanixVmApilog') {
  return page.evaluate((storagekey) => window[storagekey] ?? [], key)
}

function installapilogcapture(page, key = '__wanixVmApilog') {
...
async function openapp(page, url, log) {
...
async function dumpfailurediagnostics(page, log, apilogkey) {
  try {
