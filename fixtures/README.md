...
| Memory parity | `scripts/memory-parity-run.mjs` |
...
| Host memory repro | `scripts/build-host-memory-repro.ts` |

## Harness code (not here)

Embedded loaders and test helpers stay in `zss/testsupport/` (e.g. wanix hex
bytes). They reference sources under `fixtures/wanix/`, not duplicate assets.
