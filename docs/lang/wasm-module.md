...
Stable across all scripts. Defined in ``hostcall.ts`` and ``zss_lang_hostcall.hpp``.
...
See ``zss_lang_api.h``. Primary output is `wasm_bytes` / `wasm_bytes_len`. JS `source` is omitted in production builds.
