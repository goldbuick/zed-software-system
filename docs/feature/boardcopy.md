...
| `mapelementcopy` | `maybenew`, `from` | Copy element surface stats from one element to another, then copy kinddata fields (`category`, `kinddata`, `kindsource*`, `pushedtick`) onto `maybenew` via `memorycopyelementkinddata` |
