---
title: Wanix integration
description: Browser OS iframe, zedcafe export, task/VM rooms, terminals, and message protocol.
sidebar:
  order: 1
---
...
- [Parent vs iframe](#parent-vs-iframe)
- [Room modes and lifecycle](#room-modes--lifecycle)
- [How you start wanix](#how-you-start-wanix)
- [Zedcafe export](#zedcafe-export-the-core-loop)
- [Wasm drop path](#wasm-drop-path-task-room)
- [VM path](#vm-path)
- [findplayers flow](#findplayers-flow)
- [Terminal attach and input](#terminal-attach--input)
- [Performance](#performance-vm-boot-vs-task-drop)
- [Soft idle and warm reuse](#performance-soft-idle--warm-reuse)
- [Message protocol](#message-protocol)
- [Module map](#module-map-three-homes)
- [Gotchas](#gotchas--invariants)
- [Debugging](#debugging--validation)
- [What works today](#what-works-today-and-why)
- [zedsync](zedsync.mdx)
- [TODO](#todo-deferred--out-of-scope)
...
**Grid engine:** [`wanixtermgridstate.ts`](zss/feature/wanix/wanixtermgridstate.ts) is shared — iframe
parses ANSI into cells; parent renders snapshots from
[`wanixtermbuffer.ts`](zss/device/wanixclient/wanixtermbuffer.ts).

---

## Room modes & lifecycle

Three modes in [`wanixroomtypes.ts`](zss/feature/wanix/wanixroomtypes.ts):
...
**`mountkey`** — monotonic counter on [`WanixRoomConfig`](zss/feature/wanix/wanixroomtypes.ts). Unchanged
...
Allowlist: [`zedcafetreeschema.ts`](zss/feature/wanix/zedcafetreeschema.ts) /
[`allowed-path-patterns.json`](ops/fixtures/wanix/zedcafe/allowed-path-patterns.json).

Constants: [`wanixzedcafeconstants.ts`](zss/feature/wanix/wanixzedcafeconstants.ts).
...
[`wanixclient:exportready`](zss/device/api.ts) `{ event: 'content-ready', taskrid }`.
Parent [`handlers/exportready.ts`](zss/device/wanixclient/handlers/exportready.ts) continues the zedcafe pipeline on `wanixclient:exportready`.
...
**Steps (iframe [`runtime.ts`](zss/device/wanixserver/runtime.ts) `drop`):**
...
**Driver selection ([`wanixwasmdriver.ts`](zss/feature/wanix/wanixwasmdriver.ts)):**
...
[`wanixspawndriver.ts`](zss/device/wanixserver/spawndriver.ts). Unknown wasm throws
(no wasi default).
...
[`wanixdisplay.ts`](zss/device/wanixclient/wanixdisplay.ts).
...
Defined in [`wanixbootregression.ts`](zss/feature/wanix/wanixbootregression.ts). Any perf change must pass:
...
| Trim | Owner | Effect |
|------|-------|--------|
| Skip redundant host push when already synced | [`wanixactivateexport.ts`](zss/device/wanixclient/wanixactivateexport.ts) | Sim-fetched books already pushed by daemon |
| Push onto applyroom mount | [`wanixzedcafe.ts`](zss/device/wanixclient/wanixzedcafe.ts) `bootzedcafeexportinner` | Avoid `sync-zedcafe-halt` when mount already up |
| Phase timing | [`wanixperf.ts`](zss/feature/wanix/wanixperf.ts) | `sinceanchor` + `elapsedms` on every mark |
...
**Perf marks:** [`wanixperf.ts`](zss/feature/wanix/wanixperf.ts) logs `[wanix-perf] label {json}` in dev console
...
All Wanix device messages are emitted only via [`zss/device/api.ts`](zss/device/api.ts) helpers. Parent ↔ iframe is emit → handler → emit (no Promise RPC / once-device replies).
...
Helpers: `wanixserver*` / `wanixclient*` in [`api.ts`](zss/device/api.ts).

---

## Module map (three homes)

### Parent — [`zss/device/wanixclient/`](zss/device/wanixclient/)

| Module | Role |
|--------|------|
| [`handlers/`](zss/device/wanixclient/handlers/) (`registry.ts`, `cells`, `ready`, `exportready`, …) | `wanixclient:*` device handlers |
| [`wanixdisplay.ts`](zss/device/wanixclient/wanixdisplay.ts) | Attach / active session / tape reveal |
| [`wanixtermbuffer.ts`](zss/device/wanixclient/wanixtermbuffer.ts) (+ clipboard/scroll/text/handlers) | Parent term UI |
| [`host.tsx`](zss/screens/wanix/host.tsx) / [`wanixbridge.ts`](zss/device/wanixclient/wanixbridge.ts) | Ghost iframe + parent message bridge |
| [`wanixroom.ts`](zss/device/wanixclient/wanixroom.ts) | Room config, drop emit wrappers, VM/task API |
| [`wanixzedcafe.ts`](zss/device/wanixclient/wanixzedcafe.ts) | Parent zedcafe daemon / push / import kicks |
| [`handlers/exportready.ts`](zss/device/wanixclient/handlers/exportready.ts) | Parent export-ready continuation |
| [`handlers/menu.ts`](zss/device/wanixclient/handlers/menu.ts) | Print-only `#wanix` menu tape (`wanixclient:menu`) |
| [`wanixbindpaths.ts`](zss/device/wanixclient/wanixbindpaths.ts) | Parent bind-drop path helpers (`parse/file`) |

### Iframe — [`zss/device/wanixserver/`](zss/device/wanixserver/)

| Module | Role |
|--------|------|
| [`cafe/wanix.ts`](cafe/wanix.ts) | Thin boot: `runtime` + wanix device |
| [`wanixmenu.ts`](zss/device/wanixserver/wanixmenu.ts) / [`handlers/menu.ts`](zss/device/wanixserver/handlers/menu.ts) | Operational `#wanix` menu tape from iframe state |
| [`runtime.ts`](zss/device/wanixserver/runtime.ts) | System DOM, applyroom, terms, FS handlers |
| [`zedcafehost.ts`](zss/device/wanixserver/zedcafehost.ts) | Iframe zedcafe boot / push / binds |
| [`wanixtgzextract.ts`](zss/device/wanixserver/wanixtgzextract.ts) / [`wanixbundle.ts`](zss/device/wanixserver/wanixbundle.ts) / [`wanixcmd.ts`](zss/device/wanixserver/wanixcmd.ts) | Iframe drop staging (tgz extract, bundle flatten, task ids) |
| [`spawndriver.ts`](zss/device/wanixserver/spawndriver.ts) / [`termbridgesmoke.ts`](zss/device/wanixserver/termbridgesmoke.ts) | Spawn driver + term smoke |
| [`exportevents.ts`](zss/device/wanixserver/exportevents.ts) | Iframe `wanixclient:exportready` emit |
| [`state.ts`](zss/device/wanixserver/state.ts) + `handlers/*` | Iframe mutable state + device adapters |
| [`zss/device/wanixserver.ts`](zss/device/wanixserver.ts) | `wanixserver` device factory |

### Shared — this folder (`zss/feature/wanix/`)

| Module | Role |
|--------|------|
| [`wanixroomtypes.ts`](zss/feature/wanix/wanixroomtypes.ts) | Room / drop / menu types |
| [`wanixmenu.ts`](zss/feature/wanix/wanixmenu.ts) | Pure `#wanix` menu tape builder (server assembles state) |
| [`wanixzedcafeconstants.ts`](zss/feature/wanix/wanixzedcafeconstants.ts) / [`wanixzedcafetypes.ts`](zss/feature/wanix/wanixzedcafetypes.ts) / [`wanixzedcafewasmversion.ts`](zss/feature/wanix/wanixzedcafewasmversion.ts) | Zedcafe shared constants/types |
| [`wanixelements.d.ts`](zss/feature/wanix/wanixelements.d.ts) | Custom-element typings |
| [`wanixtermgridstate.ts`](zss/feature/wanix/wanixtermgridstate.ts) | ANSI → cell grid (iframe writes; parent renders) |
| [`wanixwasmdriver.ts`](zss/feature/wanix/wanixwasmdriver.ts) | Driver detect from wasm bytes |
| [`zedcafetreeschema.ts`](zss/feature/wanix/zedcafetreeschema.ts) / [`wanixstateexport.ts`](zss/feature/wanix/wanixstateexport.ts) / [`wanixstateimport.ts`](zss/feature/wanix/wanixstateimport.ts) | Export tree build/parse/validate |
| [`wanixperf.ts`](zss/feature/wanix/wanixperf.ts) / [`wanixbootregression.ts`](zss/feature/wanix/wanixbootregression.ts) | Perf marks / regression gate defs |
| [`zss/device/api.ts`](zss/device/api.ts) | `wanixserver*` / `wanixclient*` emit helpers |
| [`zss/device/vm/handlers/importzedcafe.ts`](zss/device/vm/handlers/importzedcafe.ts) | Sim-worker import handler |
...
[`zedcafehost.ts`](zss/device/wanixserver/zedcafehost.ts) registers that hook
...
`pushzedcafeexportlive` must import [`postwanixexportmessage`](zss/device/wanixserver/exportevents.ts) and
[`wanixperfmark`](zss/feature/wanix/wanixperf.ts) — missing imports silently broke `/zedcafe` mounts.
...
|---------|--------|----------|
| Unknown wasm driver | [`wanixwasmdriver.ts`](zss/feature/wanix/wanixwasmdriver.ts) | Throws — no default to wasi |
| Missing `#ramfs` bytes at spawn | [`wanixspawndriver.ts`](zss/device/wanixserver/spawndriver.ts) | Throws |
| Zedcafe daemon not ready | [`wanixzedcafe.ts`](zss/device/wanixclient/wanixzedcafe.ts) `ensurewanixzedcafedaemon` | Throws — drop/activate aborts |
| Export build from memory | [`readhostexportfilesfrommemory`](zss/device/wanixclient/wanixzedcafe.ts) | `buildzedcafeexportfiles()` errors propagate |
| Menu iframe timeout | [`wanixroom.ts`](zss/device/wanixclient/wanixroom.ts) | `stalled: true`, `vm: null` — no invented VM |
...
See [zedsync.mdx](zedsync.mdx) for `#wanix zedsync`, remote 9P, conflict policy, and agent peer contract.

## TODO (deferred / out-of-scope)

Follow-ups from the **less disruptive zedcafe imports** work:

- **Sim-owned flag protection (shipped)** — `isimportprotectedflagowner` in [`wanixstateimport.ts`](zss/feature/wanix/wanixstateimport.ts) skips overwrite/delete for owners ending in `_gadget`, `_chip`, `_synth`, `_layers`, or `_tracking`. Player/`pid_*` and other world flags stay importable.
