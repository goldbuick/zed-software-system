...
- `kind` is a short, stable, lowercase source/domain label -- ideally the
  emitting module or subsystem (`bridge`, `netterminal`, `format`,
...
### 3.2 `device/register/handlers/` and helpers
...
| File | Sites | Verdict | Notes |
|---|---|---|---|
| [file.ts](../../feature/parse/file.ts) | ~23 `apierror` (mostly kind `crash`), 2 kind `parsewebfile` | mostly OK; **KIND** | `crash` is the established catch-all kind (consistent). `parsewebfile` is a function-name kind -> prefer `parse`. |
...
### 3.6 `feature/synth/`, `feature/`, `memory/`, `screens/`, `device/*`, `cafe/`
...
Standardize on `apierror` and drop the hand-rolled `$red` prefixes.
...
- Non-proper-noun uppercase: `Cache Storage` in [modelcache.ts](../../feature/tts/modelcache.ts) -> lowercase.
...
2. **Channel** -- convert the F2 clipboard failure emits to `apierror`,
   removing hand-rolled `$red` prefixes.
3. **Kind** -- normalize F3 kinds (`tts`, `session`, `parse`, `download`,
   `terminal`); move session data out of the kind label.
4. **Style** -- lowercase F4 non-proper-noun words.
