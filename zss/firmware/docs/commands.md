...
| `airshare` | Optical MEMORY transfer: invite QR then stream; `receive` opens camera decode; `stop` cancels |
...
| `media` | Board TV media queue: `#media` (queue list), `#media <url>` (speaker) |
| `queue` | Media helper admin: `#queue` (menu), `#queue <peerid>` (bind), `#queue skip` / `clear` / `stop` / `limit <N>` (bridge admin) |
