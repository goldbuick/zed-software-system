...
| SYNTH (square) | Yes |
| LFSR noise (`retro`, `buzz`, `clang`, `metallic`) | Yes |
...
| FX buses / `#echo` / `#reverb` | No — Phase 3 |
| Per-voice envelope / oscillator tuning | No — Phase 3+ |
...
| `zss/feature/synth/wasm/noisewave.ts` | LFSR noise generation (parity with Tone) |
