#!/usr/bin/env bash
# Build ZSS DaisySP synth → cafe/public/daisy/
...
OUT_DIR="$REPO_ROOT/cafe/public/daisy"
...
ensureemscripten() {
  ...
  echo "  source .emsdk/emsdk_env.sh && yarn task run ops:daisy:build" >&2
  return 1
}
...
(cd "$REPO_ROOT" && yarn task run ops:daisy:bundle:processor)
