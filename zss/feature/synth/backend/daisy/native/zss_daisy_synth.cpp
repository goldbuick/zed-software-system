...
int zss_control_len() { return kControlLen; }

/** 2 = manual delay-line razzle (post–DaisySP Chorus) */
float zss_razzle_tag() { return 2.f; }
...
void zss_process(float* out, int frames, const float* tts_in) {
  ...
  for (int f = 0; f < frames; ++f) {
    ...
    float key = sidechainkey(bg * kVoiceOutGain * g_engine.cached_bggain,
                             ttssample, g_engine.drumsidechain);
    sidechainupdate(key);
