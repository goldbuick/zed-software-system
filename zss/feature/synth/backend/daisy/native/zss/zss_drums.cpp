...
float drumnotelen(int n) { return (0.5f * 4.f) / static_cast<float>(n); }
...
float drumnoise() { return g_engine.drum_whitenoise.Process(); }
...
float drumtick() {
  ...
  float amp =
      drumadsr(age, drumsamp(0.001f), drumsamp(0.05f), 0.001f, drumsamp(0.05f));
  if (amp <= 0.f) {
    return 0.f;
  }
...
float drumtweet() {
  ...
  float sr = g_engine.sample_rate;
  float amp =
      drumadsr(age, drumsamp(0.001f), drumsamp(0.16f), 0.06f, drumsamp(0.18f));
  if (amp <= 0.f) {
    return 0.f;
  }
  static BiquadCoef hp, lo, mid, hi;
...
float drumcowbell() {
  ...
  float sr = g_engine.sample_rate;
  float syn =
      drumadsr(age, drumsamp(0.001f), drumsamp(0.01f), 0.1f, drumsamp(0.1f));
  float genv =
      0.5f *
      std::pow(0.02f, std::min(1.f, static_cast<float>(age) / drumsamp(0.35f)));
  float amp = syn * genv;
  if (amp <= 0.f) {
    return 0.f;
  }
...
float drumclap() {
  ...
  float sr = g_engine.sample_rate;
  float amp =
      drumadsr(age, drumsamp(0.01f), drumsamp(0.1f), 0.1f, drumsamp(0.1f));
  if (amp <= 0.f) {
    return 0.f;
  }
  static BiquadCoef hp, lo, mid, hi;
...
float drumwoodblock(bool hi) {
  ...
  float sr = g_engine.sample_rate;
  float clackamp = drumadsr(age, drumsamp(0.001f), drumsamp(0.001f), 0.001f,
                            drumsamp(0.08f));
  float donkamp =
      drumadsr(age, drumsamp(0.001f), drumsamp(0.1f), 0.001f, drumsamp(0.08f));
  ...
  float clackhz = drumexpramp(age, drumsamp(drumnotelen(32)),
                              hi ? 2000.f : 2000.f, clackend);
  float donkhz =
      drumexpramp(age, drumsamp(drumnotelen(256)), hi ? 999.f : 699.f, donkend);
...
float drumcymbalmodal(int idx, int age, int len, const float ratios[6],
  ...
  float burst =
      drumadsr(age, drumsamp(0.001f), drumsamp(0.04f), 0.001f, drumsamp(0.06f));
