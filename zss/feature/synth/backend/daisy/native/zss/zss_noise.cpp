...
void initnoisetables() {
  ...
  drawnoisespectrum(g_noise_hollow, g_noise_lfsr, 11, 14, 0.6578f, 0.6578f,
                    0.f);
...
const float* noiseforvoice(int noisetype) {
  ...
  case kBuzzNoise:
    return g_noise_buzz;
  case kClangNoise:
    return g_noise_clang;
  case kMetallicNoise:
    return g_noise_metallic;
  case kHollowNoise:
    return g_noise_hollow;
  case kWhiteNoise:
    return g_noise_white;
  default:
    return g_noise_lfsr;
  }
}
