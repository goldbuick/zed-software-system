...
float synthsource(ZssVoice& v, int vi, float freq, bool gate, float detune, int osctype,
...
float algovoice(ZssVoice& v, int vi, float freq, bool gate, int algo,
                float vfreq[kVibratoGroups]);
float noisevoice(ZssVoice& v, int vi, int noisetype, float freq, bool gate, float envout);
float stringbownoisesample(ZssVoice& v);
void applypluckparams(ZssVoice& v, int cfg);
void applystringensembleparams(ZssVoice& v, int cfg, float& detunecents, float& pwmdepth,
                               float& vibcents, float& filterscale);
