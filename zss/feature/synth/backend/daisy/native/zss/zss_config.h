...
constexpr int kControlLen  = kVoicesLen + kDrumsLen + kMainLen + kFxLen + kVoiceCfgLen
                           + kOscCfgLen + kAlgoCfgLen + kVibratoLen;
...
constexpr float kVoiceOutGain = 0.67f;
/** Tone playvolume: volumetodb(20); drumvolume: volumetodb(100) + 10 dB */
constexpr float kPlayBusGain = 0.21f;
constexpr float kDrumBusGain = 0.36f;
...
constexpr float kReverbPostGain = 1.6f;
