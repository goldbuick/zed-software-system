...
export function initdaisyvoicesab(maxi: SabEngine) {
...
export function initdaisydrumsab(maxi: SabEngine, strikes?: number[]) {
  pushwasmsabvalues(
    maxi,
    WASM_DRUMS_SAB,
...
export type DAISY_RECORD_FACTORY = (
  maxi: SabEngine,
  recording: RECORDING_STATE,
  deps: DAISY_RECORD_DEPS,
) => {
  synthrecord: (filename: string) => void
  synthflush: () => void
}

export function createdaisysynth(
  maxi: SabEngine,
  hooks: DAISY_SYNTH_HOOKS = {},
  recordfactory?: DAISY_RECORD_FACTORY,
) {
  initwasmsabchannels(maxi)
  initdaisyvoicesab(maxi)
  initwasmfxsab(maxi)
  initwasmvibratosab(maxi, maxi.audioContext.currentTime)
  initdaisydrumsab(maxi)
...
export type DAISY_SYNTH = ReturnType<typeof createdaisysynth>
