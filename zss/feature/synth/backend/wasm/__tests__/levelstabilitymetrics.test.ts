...
describe('levelstabilitymetrics', () => {
  it('reports low peak range for steady sine', () => {
    const metrics = analyzelevelstability(rendersine(440, 1, 0.25), SAMPLERATE, 46)
  ...
  it('reports higher peak range for amplitude-modulated sine', () => {
    const steady = analyzelevelstability(rendersine(440, 1, 0.25), SAMPLERATE, 46)
    const modulated = analyzelevelstability(renderamplitude(8, 1), SAMPLERATE, 46)
  ...
  it('formats a report line', () => {
    const metrics = analyzelevelstability(rendersine(440, 0.5, 0.1), SAMPLERATE, 46)
  ...
  it('formats a side-by-side window compare plot', () => {
    const steady = analyzelevelstability(rendersine(440, 1, 0.25), SAMPLERATE, 46)
    const modulated = analyzelevelstability(renderamplitude(8, 1), SAMPLERATE, 46)
    const plot = formatwindowcompareplot('steady', 'modulated', steady, modulated, 8)
