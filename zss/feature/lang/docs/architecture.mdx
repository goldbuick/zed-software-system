...
1. **Incomplete direction mod at EOL** — e.g. `#put opp` → `direction incomplete: opp needs a base dir (...)`
2. **Unclosed block** — `#if` / `#while` / `#repeat` / `#foreach` / `#try` without `#do` on the line → `#if block needs #do ... #done`
3. **Label vs command** — `:foo` where a `#foo` command was expected
4. **Redundant input** — extra tokens after a complete statement
5. **Direction / color / statement / value** — long `Expecting [token_*]` lists collapse to short category hints
6. **Fallback** — `unexpected 'foo' here` or `syntax error near this spot` (capped at 120 chars, ASCII only)

Editor display ([`editorrows.tsx`](../../../screens/editor/editorrows.tsx)) and compile failures ([`os.ts`](../../../os.ts) tape preamble) consume formatted `LANG_ERROR.message` unchanged.
...
| Error messages | [formatlangerror.md](formatlangerror.md) |
