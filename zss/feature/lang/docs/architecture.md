...
2. **`parser.program()`** ([parser.ts](../backend/typescript/parser.ts)) — Concrete syntax tree (`CstNode`). Parser errors map to [`LANG_ERROR`](../backend/typescript/lexer.ts)-shaped messages in [ast.ts](../backend/typescript/ast.ts).
...
| ------- | ----- | ------------ |
| Lexical | `tokenize` | [ast.ts](../backend/typescript/ast.ts) returns `errors` from lexer; no parse |
...
[editor component.tsx](../../../screens/editor/component.tsx) uses `compileast`, lexer, [`createlineindexes`](../backend/typescript/transformer.ts), and `CodeNode` / `NODE` for parsing and structure without always running full `compile` / `new Function` on every edit.
...
- **Newlines and `#do` / `#done`** — Block forms ([`command_if_block`](../backend/typescript/parser.ts), etc.) expect specific newline/`#` structure; easy to get opaque parse errors.
