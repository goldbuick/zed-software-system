...
private:
  ...
  void emitapiwords(int hostindex,
                    const std::vector<std::unique_ptr<CodeNode>>& words) {
    trackhost(hostindex);
  ...
  void emitapiwords(const std::string& method,
                    const std::vector<std::unique_ptr<CodeNode>>& words) {
  ...
  void emitcompare(CodeNode* ast) {
    ...
    case COMPARE::IS_LESS_THAN:
      host = HOST_IS_LESS_THAN;
      break;
    case COMPARE::IS_GREATER_THAN:
      host = HOST_IS_GREATER_THAN;
      break;
    case COMPARE::IS_LESS_THAN_OR_EQ:
      host = HOST_IS_LESS_THAN_OR_EQ;
      break;
    case COMPARE::IS_GREATER_THAN_OR_EQ:
      host = HOST_IS_GREATER_THAN_OR_EQ;
      break;
    }
    trackhost(host);
  ...
  void emitoperator(CodeNode* ast) {
    ...
    for (auto& item : ast->items) {
      ...
      case OPERATOR::MULTIPLY:
        host = HOST_OP_MULTIPLY;
        break;
      case OPERATOR::DIVIDE:
        host = HOST_OP_DIVIDE;
        break;
      case OPERATOR::MOD_DIVIDE:
        host = HOST_OP_MOD_DIVIDE;
        break;
      case OPERATOR::FLOOR_DIVIDE:
        host = HOST_OP_FLOOR_DIVIDE;
        break;
      case OPERATOR::UNI_PLUS:
        host = HOST_OP_UNI_PLUS;
        break;
      case OPERATOR::UNI_MINUS:
        host = HOST_OP_UNI_MINUS;
        break;
      default:
        continue;
      }
  ...
  void emitwasmnode(CodeNode* ast) {
    ...
    case NODE::PROGRAM: {
      ...
      for (auto& line : ast->lines)
        emitwasmnode(line.get());
