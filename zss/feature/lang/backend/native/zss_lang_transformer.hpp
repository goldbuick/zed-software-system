...
class Transformer {
 ...
 private:
  ...
  void writeadd(SourceNode& node, SourceNode child) { node.add(std::move(child)); }

  SourceNode write(const CodeNode* ast, std::initializer_list<SourceNode> children) {
    SourceNode node(nodeline(ast), nodecol(ast), GENERATED_FILENAME);
    for (auto& c : children) node.add(std::move(c));
    return node;
  }
  ...
  std::vector<SourceNode> transformnodeslist(const std::vector<std::unique_ptr<CodeNode>>& nodes) {
    std::vector<SourceNode> out;
    for (const auto& n : nodes) {
      if (n) out.push_back(transformnode(n.get()));
    }
    return out;
  }
  ...
  SourceNode transformcompare(const CodeNode* ast) {
    if (!ast || ast->type != NODE::COMPARE || !ast->compare) return blanknode(ast);
    const char* method = nullptr;
    switch (ast->compare->compare_method) {
      case COMPARE::IS_EQ: method = "isEq"; break;
      case COMPARE::IS_NOT_EQ: method = "isNotEq"; break;
      case COMPARE::IS_LESS_THAN: method = "isLessThan"; break;
      case COMPARE::IS_GREATER_THAN: method = "isGreaterThan"; break;
      case COMPARE::IS_LESS_THAN_OR_EQ: method = "isLessThanOrEq"; break;
      case COMPARE::IS_GREATER_THAN_OR_EQ: method = "isGreaterThanOrEq"; break;
    }
    if (!method) return blanknode(ast);
    return writeapi(ast, method, {transformnode(ast->lhs.get()), transformnode(ast->rhs.get())});
  }
  ...
  void transformoperatoritem(const CodeNode* item, SourceNode& operation) {
    if (!item || item->type != NODE::OPERATOR_ITEM) return;
    switch (item->operator_kind) {
      case OPERATOR::PLUS: prefixapi(operation, "opPlus", item->rhs.get()); break;
      case OPERATOR::MINUS: prefixapi(operation, "opMinus", item->rhs.get()); break;
      case OPERATOR::POWER: prefixapi(operation, "opPower", item->rhs.get()); break;
      case OPERATOR::MULTIPLY: prefixapi(operation, "opMultiply", item->rhs.get()); break;
      case OPERATOR::DIVIDE: prefixapi(operation, "opDivide", item->rhs.get()); break;
      case OPERATOR::MOD_DIVIDE: prefixapi(operation, "opModDivide", item->rhs.get()); break;
      case OPERATOR::FLOOR_DIVIDE: prefixapi(operation, "opFloorDivide", item->rhs.get()); break;
      case OPERATOR::UNI_PLUS: prefixuniapi(operation, "opUniPlus", item->rhs.get()); break;
      case OPERATOR::UNI_MINUS: prefixuniapi(operation, "opUniMinus", item->rhs.get()); break;
      default: break;
    }
  }

  SourceNode transformoperator(CodeNode* ast) {
    if (!ast || ast->type != NODE::OPERATOR) return blanknode(ast);
    SourceNode operation = ast->lhs ? transformnode(ast->lhs.get()) : write(ast, "");
    for (auto& item : ast->items) {
      if (item) transformoperatoritem(item.get(), operation);
    }
    return operation;
  }
  ...
  SourceNode transformnode(CodeNode* ast) {
    ...
    switch (ast->type) {
      case NODE::PROGRAM: {
        ...
        for (auto& l : ast->lines) node.add(transformnode(l.get()));
        node.add("default:\n");
        node.add("  return 0;\n");
        node.add("}\n");
      ...
      case NODE::LINE: {
        ...
        for (auto& s : ast->stmts) node.add(transformnode(s.get()));
        node.add("  break;\n");
        return node;
      }
      case NODE::MARK:
      ...
      case NODE::TEXT: {
        SourceNode node(nodeline(ast), nodecol(ast), GENERATED_FILENAME);
        node.add("  ");
        node.add(writeapi(ast, "text", {write(ast, writetemplatestring(ast->value))}));
        node.add(";\n");
        return node;
      }
      case NODE::STAT:
      ...
      case NODE::LABEL: {
        ...
        return write(ast, "  // " + std::to_string((ast->active ? 1 : -1) * ast->lineindex) + " '" +
                           llabel + "' " + (ast->active ? "label" : "comment") + "\n");
      }
      case NODE::HYPERLINK: {
        SourceNode node(nodeline(ast), nodecol(ast), GENERATED_FILENAME);
        ...
        std::string part;
        while (iss >> part) node.add(", " + writestring(part));
        node.add(");\n");
        return node;
      }
      case NODE::MOVE: {
      ...
      case NODE::IF: {
        ...
        if (block && block->type == NODE::IF_BLOCK) {
          ...
          for (auto& l : block->lines) source.add(transformnode(l.get()));
          writelookupline(ptrs(block->altlines), NODE::ELSE_IF, readlookup(block->done));
          for (auto& l : block->altlines) source.add(transformnode(l.get()));
          return source;
        }
      ...
      case NODE::ELSE: {
        ...
        for (auto& l : ast->lines) source.add(transformnode(l.get()));
        return source;
      }
      case NODE::WHILE: {
