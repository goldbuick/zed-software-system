...
**Purpose**: Heavy processing workloads — TTS engines (Piper, Supertonic), Qwen2.5-0.5B LLM (ONNX/WebGPU), agent with tool calls. Runs in main thread or workers; loads models on demand.
...
| `model.ts` | Qwen2.5-0.5B-Instruct ONNX via AutoModelForCausalLM; `modelgenerate`, `modelclassify`, `parseresult` (XML/JSON + Pythonic fallback) |
...
| `modelgenerate` | Qwen2.5-0.5B-Instruct text generation with tool calls |
