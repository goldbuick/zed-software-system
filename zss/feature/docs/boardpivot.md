...
- **Group path terrain:** After placing group tiles at pivoted destinations via `deepcopy` (fresh element objects, including inline `kinddata` / `category`), terrain on **incoming-only** indices (destinations that are not former group sources) is copied into **vacated** former group cells (sorted by linear index, pairwise), so non-group terrain is not deleted — same idea as `boardweavegroup`
