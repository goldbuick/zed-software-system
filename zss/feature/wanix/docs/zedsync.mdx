...
mtime** wins (`copytoz` or `copytor`). Equal mtime ties prefer **zedcafe**
(`copytor`) so remount/equalized peer mtimes cannot stomp live export. Peer
still wins when its mtime is **strictly newer**. Live player flag bags
(`flags/pid_*.json`) and board player objects (`board/objects/pid_*.json`)
never `copytoz` into `zedcafe/` (peer can still receive updates via
`copytor`). Board player objects **are** walked so when export drops an
avatar, `SteadyTick` runs `deleteremote` and prunes the peer orphan. There
is **no merge**.
...
After seed, zedsync runs one **post-seed full tick**, then mostly incremental
ticks with a **periodic full tick** every 30 polls so peer↔`zedcafe/` drift
heals without emptying the peer.
...
and the copy completing) is best-effort re-copied. `zedsync/` is skipped by
name in `WalkFiles` (same as dot-prefixed segments) — it never appears as a
syncable path. Delete the peer's `zedsync/` folder to clear journal / ready
