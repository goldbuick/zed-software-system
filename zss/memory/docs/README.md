...
The previous `index.ts` was split into smaller files (`session.ts`, `boards.ts`, `books.ts`, `bookoperations.ts`, `boardaccess.ts`, `boardlifecycle.ts`, `boardelement.ts`, …). For a flat function-by-function catalog see [`EXPORTED_FUNCTIONS.md`](../EXPORTED_FUNCTIONS.md). The narrative docs below cover the modules with bigger surface area.
