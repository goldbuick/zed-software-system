...
| Export | Description |
|--------|-------------|
| `memorymoveplayertoboard(book, player, board, dest)` | Authoritative move; called from firmware `#goto`, edge exits, and [`vm:playermovetoboard`](../../device/vm/handlers/playermovetoboard.ts) for main-thread callers. Returns `false` if the destination is blocked. |
...
| `memoryreadbookplayerboards(book)` | Returns every board (over board first when present) that has at least one active player. Used by `handleticktock` and `gadgetsynctick`. |
