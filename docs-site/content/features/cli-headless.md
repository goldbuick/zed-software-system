...
|---------|----------|-------------|---------|
| zss oclif binary | Dev | Run cafe SPA in Playwright with Ink terminal overlay. | `ops/headless/src/commands/run.ts` |
| Headless boot | Dev | bootheadless() skips Canvas; same message stack. | `cafe/index.tsx` |
| Node storage hooks | Dev | CLI injects __nodeStorageReadPlayer for persistence. | `ops/headless/src/lib/app.tsx` |
