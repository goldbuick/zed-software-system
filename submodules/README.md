...
Parent pins a **submodule commit** that forwards `{ zedcafeexportdirty: true }` from gojs tasks to `__wanixOnZedcafeExportDirty` in [`web/worker/worker.go`](wanix/web/worker/worker.go). That commit is **not** on upstream `tractordev/wanix`.
...
4. Toolchain probe (`ops:fixtures:wanix:toolchains`) and zedcafe build fail loudly if the marker `__wanixOnZedcafeExportDirty` is missing from `worker.go`.
