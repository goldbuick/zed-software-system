...
| Pattern | Risk | Fix |
|---------|------|-----|
| `loadscriptsync(...).run()` with hand-rolled stub chip | Sync infinite `while(true)` | Use `createwasmstubchip()` from `ops/tests/lib/lang/wasmruntestutil.ts` |
...
- Global teardown clears modem Awareness: `ops/jest/globalteardown.cjs`
